...
@objc private extension RulerARProViewController {
    ...
    func finishAreaAction(_ sender: UIButton) {
        ...
        for l in lineSets {
            points = l.lines.map({ $0.endNode.position })
            points.append(l.lines[0].startNode.position)
            
            let myPoint = Point()
            myPoint.x = l.lines[0].startNode.position.x
            myPoint.y = l.lines[0].startNode.position.y
            myPoint.z = l.lines[0].startNode.position.z
        ...
        DispatchQueue(label: "background").async {
            autoreleasepool {
                let realm = try! Realm()
                let theDog = realm.objects(Point.self)
                print(theDog.x," ",theDog.y," ",theDog.z)
            }
        }
